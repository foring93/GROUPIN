	...
	if (answer) {
		$.ajax({
			...
			success : function(data) {
				...
				$('.coment-area').empty();
				$('.comment ins').text('').text(data.listcount);
...
$(function() {
	...
	$(document).on('click', '.secretbtn>i', function() {
		if (!lock) { // 비댓
			console.log('비댓');
	...
	$(document).on('submit', $('#ReplyAction'), function() {
		...
		console.log('대댓글일 시 원댓의 댓글키값 = ' + $('input[name="commentNum"').val());
		console.log('비댓여부(show) = ' + $('input[name=commentshow]').val());
